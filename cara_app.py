...
2. Place these files in the same folder as this script:
   ...
   - faiss.index
   - mtsamples.csv
3. Add this in Streamlit Cloud secrets:
...
def generate_explanation(prompt: str, api_key: str | None = None) -> str:
    ...
    if not api_key:
        raise ValueError(
            "Gemini API key not found. Add GEMINI_API_KEY in Streamlit secrets before running the app."
        )
...
if not get_gemini_api_key():
    st.warning(
        "Gemini API key not found in Streamlit secrets. Add GEMINI_API_KEY in your Streamlit app settings to enable AI explanations."
    )
...
- MTSamples Knowledge Base
- Features entered via sidebar controls
- Top 3 relevant notes from MTSamples
- AI explanation grounded in retrieved evidence
""")

    st.info("Streamlit Cloud: add GEMINI_API_KEY in App Settings → Secrets")

st.title("CARA — Cardiovascular AI Risk Advisor")
st.caption("Updated for Streamlit Cloud secrets support.")
